...
public final class MusicSheet extends CoreItem {
  ...
  private static final TranslatableComponent music_sheet_clear    = new TranslatableComponent("gui.addsynthcore.music_sheet.clear");
  private static final TranslatableComponent music_sheet_paste    = new TranslatableComponent("gui.addsynthcore.music_sheet.paste");
  private static final TranslatableComponent music_sheet_copy     = new TranslatableComponent("gui.addsynthcore.music_sheet.copy");
  private static final TranslatableComponent music_sheet_no_data  = new TranslatableComponent("gui.addsynthcore.music_sheet.no_data");
  private static final TranslatableComponent music_sheet_has_data = new TranslatableComponent("gui.addsynthcore.music_sheet.has_data");
  ...
  public final InteractionResultHolder<ItemStack> use(final Level world, final Player player, final InteractionHand hand){
    ...
    if(world.isClientSide == false){
      if(player.isCrouching()){
        stack.setTag(null);
        MessageUtil.send_to_player(player, music_sheet_clear);
      ...
      if(tile != null && player != null){
        if(player.isCrouching() == false){
          ...
          if(nbt != null){
            ...
            tile.changed = true;
            MessageUtil.send_to_player(player, music_sheet_paste);
            return InteractionResult.SUCCESS;
          }
        }
        // is sneaking
  ...
  private static final void copy_music_data(final ItemStack stack, final Player player, final TileMusicBox tile){
    ...
    MessageUtil.send_to_player(player, music_sheet_copy);
  }
